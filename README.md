...
* **[Architecture & Memory Model](docs/ARCHITECTURE.md)**: A deep dive into the Zig-to-C99 transpilation pipeline, the 4GB `mmap` Virtual Arena, and Canonical Module Linking.
* **[Contributing Guide](docs/CONTRIBUTING.md)**: Learn how to build the compiler from source, run the `flint test` orchestrator, and adhere to our strict C99/Zig coding standards.
...
* **Pipeline Safety Primitives:** Errors are intercepted inline. Use `~> expect("msg")` for controlled panics, or `~> fallback(default_val)` for silent data rescues without breaking the visual flow.
* **Clean Imports & Canonical Linking:** Use `import os;` to natively resolve standard libraries. The linker automatically deduplicates module aliases, ensuring binary size remains microscopic.
...
*Fragile Bash relies on external binaries and fails silently if pipes break. Flint is native, safe, and stops immediately on critical errors.*
...
const username = os.env("USER");

print("Collecting processes...");

os.exec("ps aux")
    ~> strings.lines()
    ~> strings.grep(username)
    ~> strings.join("\n")
    ~> io.write_file("user_procs.log")
    ~> expect("Fatal: Could not write log to disk.");
...
*Python's dynamic nature makes JSON parsing slow and error-prone. Flint defines rigid memory structs and parses massive network payloads in microseconds, falling back to local caches safely.*
...
# Native HTTP with fallback to local cache if the network fails
const payload = http.fetch("https://api.github.com/users/lucaas-d3v")
    ~> fallback(io.read_file("cache.json"))
    ~> expect("Network Failure and Cache Missing.");

# Zero-copy, AOT-compiled mapping from JSON to native C struct
const user = parse_json_as(GithubUser, strings.to_str(payload));

print(strings.concat("Developer: ", user.login));
```
...
**Verdict:** Pydantic requires parsing the entire JSON into a dynamic dictionary of PyObjects before applying runtime reflection to validate fields. Flint generates native C deserializers at compile-time, slicing the mmap buffer directly into physical struct offsets in O(1) time.
...
The Flint compiler is written in [Zig](https://ziglang.org) and bootstraps itself. You need Zig `0.15.2` and `clang` (or `gcc`) installed on your system.
...
1. Download the `flint-lang-1.7.2.vsix` file from the [Releases page](https://github.com/lucaas-d3v/flint/releases).
